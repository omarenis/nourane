...
def extract_data_with_validation(request, fields: dict) -> dict or Exception:
    ...
    for i in data:
        ...
        elif data.get(i) is None and fields.get(i).get('required') is True:
            return Exception(f'{i} is required')
        elif fields.get(i) is not None and fields.get(i).get('type') == 'email':
            try:
                validate_email(data.get(i))
            except ValidationError:
                return ValueError(f"{data.get(i)} does not have an email format")
        output[i] = data.get(i)
    return output
